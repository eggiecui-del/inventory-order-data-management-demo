...
def build_database_url(dbname="inventory_order_demo"):
    ...
    return psycopg.conninfo.make_conninfo(
        ...
        dbname=os.environ.get("POSTGRES_APP_DB", dbname),
    )
...
def _pg_sql(sql: str) -> str:
    # Keep the SQL in the app readable while sending psycopg-style placeholders.
...
def clear_demo_data(database_url=None):
    with get_connection(database_url) as conn:
        conn.execute(
            """
            TRUNCATE TABLE
...
def get_product_id_by_code(conn, product_code):
    row = conn.execute(
